...
let runUpgrades = []; // 永久追蹤這局遊戲的所有升級進度與星級
...
function createEnemy(typeKey) {
    ...
    enemies.push({ 
        ...
        x: x, 
        y: y, 
        currentHp: (data.hp + scaling) * chapMult * timeMult, 
        maxHp: (data.hp + scaling) * chapMult * timeMult,
        damage: data.damage * chapMult * timeMult, 
        exp: data.exp, 
        lastAttack: 0 
    });
}
...
function update(dt) {
    ...
    while (exp >= expToNext) {
        ...
        if (availablePool.length > 0) {
            ...
            let autoGold = Math.floor(mins * 0.8 * 100);
