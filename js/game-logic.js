...
function handleEndGame(isWin = false, isSurrender = false) {
    ...
    // 試煉模式結算金幣乘以 3
    const trialGoldMult = isTrialMode ? 3 : 1;
...
function createEnemy(typeKey) {
    ...
    const trialMult = isTrialMode ? 5 : 1; // 試煉模式乘以 5
    const elapsedSecs = Math.floor(totalFrames / 60);
    const elapsedMins = Math.floor(elapsedSecs / 60);
    ...
    enemies.push({ 
        ...
        x: x, 
        y: y, 
        currentHp: (data.hp + scaling) * chapMult * timeMult * trialMult, 
        maxHp: (data.hp + scaling) * chapMult * timeMult * trialMult,
        damage: data.damage * chapMult * timeMult * trialMult, 
        exp: data.exp, 
        lastAttack: 0 
    });
}
...
function update(dt) {
    ...
    while (exp >= expToNext) {
        ...
        if (availablePool.length > 0) {
            ...
            // 試煉模式自動轉化金錢亦乘以 3
            let autoGold = Math.floor(mins * 0.8 * 100) * (isTrialMode ? 3 : 1);
    ...
    const chapMult = chapterData[selectedChapter - 1].multiplier;
    const trialMult = isTrialMode ? 5 : 1; 
    const elapsedMins = Math.floor(elapsedSecs / 60);
    const timeMult = 1 + (0.5 * elapsedMins);
        ...
        if (en.type === 'sniperBoss') {
            ...
            if (now - en.lastAttack > 2500) {
                ...
                enemyProjectiles.push({ 
                    ...
                    color: '#ff4d4d', 
                    size: 8, 
                    damage: 25 * chapMult * timeMult * trialMult 
                });
                ...
                for(let i = 0; i < 4; i++) {
                    ...
                    enemyProjectiles.push({ 
                        ...
                        color: '#ffff00', 
                        size: 10, 
                        damage: 40 * chapMult * timeMult * trialMult 
                    });
                }
            }
        } else {
